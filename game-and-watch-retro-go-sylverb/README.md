...
> GW Studio release note: ColecoVision (`col`) does not ship with a BIOS. GW Studio asks the user to provide `coleco.rom`, verifies its size and SHA-1, and generates `coleco_bios.h` only in the temporary build workspace. Amstrad CPC (`amstrad`) is disabled/removed in this bundled copy because its upstream support depended on ROM files that are not shipped with GW Studio. Users must provide their own ROM/BIOS/firmware files where required.
...
SMW and Zelda3 direct-loader ports are removed from the GW Studio release copy. GW Studio currently builds only the supported emulator cores and skips .sfc direct-loader imports.
